...
- (void)loadUIWithDataModel:(ZZFDGoodListModel *)listModel
{
    ...
    if (listModel.params.count > 0) {
       ...
       /** 填补空白色 */ self.addCell(@"ZZFDGoodParamCell").toSection(ZZFDGoodSectionTypeDetail);
        }
        self.addSeperatorCell(CGSizeMake(-1, 8), [UIColor redColor]).toSection(ZZFDGoodSectionTypeDetail);
    }
    // 详细描述
    ...
    if (listModel.goodImages.count > 0) {
        ...
        self.addSection(ZZFDGoodSectionTypeImage).sectionInsets(UIEdgeInsetsMake(8, 15, 20, 15)).minimumLineSpacing(10).minimumInteritemSpacing(10).backgrounColor([UIColor yellowColor]);
        ...
        if (listModel.goodImages.count <= 4) {
            ...
            if (listModel.goodImages.count <= 9) {
               ...
               /** 添加更多 */ self.addCell(@"ZZFDImageMoreCell").toSection(ZZFDGoodSectionTypeImage).selectedAction(^(id data) {
                    @strongify(self);
